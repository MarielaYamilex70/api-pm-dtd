...
class ScheduleController extends Controller
{
    ...
    public function countScheduleXCoder()
    {
            
        $match = DB::select('CALL getCountScheduleXCoder(1)');    
        //dd($match);
        if ($match) {
            $data = [
                'message' => 'Maximum fetched successfully',
                'match' => $match
            ];
            return response()->json($data);
        }
        else{
            return response()->json(['message' => 'Error to fetched Maximum'], 500);     
        }
    }

    public function countScheduleXRecruiter()
    {
            
        $match = DB::select('CALL getCountScheduleXRecruiter(1)');    
